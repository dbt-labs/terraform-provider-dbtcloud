...
func TestAccDbtCloudProfilesDataSource(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDbtCloudProfilesDataSourceConfig(projectName, profileKey),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"data.dbtcloud_profiles.test",
						"profiles.#",
						"1",
					),
				),
...
func testAccDbtCloudProfilesDataSourceConfig(projectName, profileKey string) string {
	return fmt.Sprintf(`
...
resource "dbtcloud_global_connection" "test_connection" {
  name = "profile_all_ds_test_connection_%s"
