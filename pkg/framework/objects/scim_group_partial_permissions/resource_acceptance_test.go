...
// TestDbtCloudScimGroupPartialPermissionsResource_ValidateConfig tests that the resource
// configuration is valid and can be planned. We use ExpectNonEmptyPlan because
// the resource would create new infrastructure.
func TestDbtCloudScimGroupPartialPermissionsResource_ValidateConfig(t *testing.T) {
...
func testAccDbtCloudScimGroupPartialPermissionsResourceBasicConfig() string {
	return `
resource "dbtcloud_project" "test_project" {
  name = "Test Project for SCIM Group Partial Permissions"
}

# Example SCIM group partial permissions configuration
# In practice, the group_id would be from an external identity provider
# and retrieved using the dbtcloud_groups data source
resource "dbtcloud_scim_group_partial_permissions" "test" {
