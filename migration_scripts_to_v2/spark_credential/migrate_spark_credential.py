#!/usr/bin/env python3
"""
Migration script: dbtcloud_spark_credential changes (v1 -> v2)

Changes in v2:
  - resource "dbtcloud_spark_credential": `target_name` attribute removed.
  - data "dbtcloud_spark_credential": `target_name` attribute removed.

This script removes `target_name = ...` lines from all matching blocks.

Usage:
    python migrate_spark_credential.py [--dry-run] <path> [<path> ...]

    <path> can be a .tf file or a directory (searched recursively).

Examples:
    python migrate_spark_credential.py ./terraform/
    python migrate_spark_credential.py --dry-run ./envs/prod/ ./envs/staging/
    python migrate_spark_credential.py module1/main.tf module2/main.tf
"""
...
def process_file(path: Path, dry_run: bool) -> bool:
    ...
    content, n = remove_attribute_from_blocks(content, "resource", "dbtcloud_spark_credential", "target_name")
    if n:
        changes.append(f"  [REMOVE] {n} `target_name` attribute(s) from resource \"dbtcloud_spark_credential\" block(s)")

    content, n = remove_attribute_from_blocks(content, "data", "dbtcloud_spark_credential", "target_name")
    if n:
        changes.append(f"  [REMOVE] {n} `target_name` attribute(s) from data \"dbtcloud_spark_credential\" block(s)")
