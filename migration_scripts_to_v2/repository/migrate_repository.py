#!/usr/bin/env python3
"""
Migration script: dbtcloud_repository changes (v1 -> v2)

Changes in v2:
  - resource "dbtcloud_repository": the `fetch_deploy_key` attribute has been removed.
  - data "dbtcloud_repository": the `fetch_deploy_key` attribute has been removed.

This script removes `fetch_deploy_key = ...` lines from all dbtcloud_repository
resource and data source blocks.

Usage:
    python migrate_repository.py [--dry-run] <path> [<path> ...]

    <path> can be a .tf file or a directory (searched recursively).

Examples:
    python migrate_repository.py ./terraform/
    python migrate_repository.py --dry-run ./envs/prod/ ./envs/staging/
    python migrate_repository.py module1/main.tf module2/main.tf
"""
...
def process_file(path: Path, dry_run: bool) -> bool:
    ...
    content, n = remove_attribute_from_blocks(content, "resource", "dbtcloud_repository", "fetch_deploy_key")
    if n:
        changes.append(f"  [REMOVE] {n} `fetch_deploy_key` attribute(s) from resource \"dbtcloud_repository\" block(s)")

    content, n = remove_attribute_from_blocks(content, "data", "dbtcloud_repository", "fetch_deploy_key")
    if n:
        changes.append(f"  [REMOVE] {n} `fetch_deploy_key` attribute(s) from data \"dbtcloud_repository\" block(s)")
